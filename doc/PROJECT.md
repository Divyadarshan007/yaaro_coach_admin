...
- ~~Coach auth exists now (§10), but the Program/Routine endpoints aren't gated behind it~~ — **resolved 2026-08-13**: `/coach/v1/programs*` and `/coach/v1/routines*` are now behind `authenticateCoachToken`; `createdById` is always `req.coachToken.id`, the `DEFAULT_COACH_ID` stub is gone. See §9.
...
- **As of 2026-08-13, `program` and `routine` are two separate top-level collections, not one embedded document, and every route requires real coach auth** (`authenticateCoachToken`, `createdById: req.coachToken.id` — the old `DEFAULT_COACH_ID` stub is gone). `program`: `title, notes, image, duration, level, goal, equipment, createdById (ref user), visibility (public|private), routineIds [ref routine]`. `routine`: `title, notes, createdById, visibility, exercises: [{exerciseId (ref exercise, required), notes, restSeconds, actions:[{key,data}], set:[{metrics:[{type,value}]}]}]`. The old `programTemplate` model is **gone** — a "template" is now just a `program` doc with `visibility: "public"` owned by someone other than the viewing coach; `GET /coach/v1/programs`/`/routines` return the caller's own docs plus everyone's public ones. Seeded via `yaaro_backend/scripts/seed-public-programs.js` (renamed from `seed-program-templates.js`, `npm run seed:publicPrograms`, requires `SEED_PROGRAM_OWNER_ID` env var). Full detail + the clone-vs-share-by-reference rule in the `yaaro-program-architecture` memory.
...
- **Store split (2026-08-13):** `useMyProgramsStore` now only owns program-level scalar fields + `routineIds` (debounce-persisted via `updateProgramAction`/`updateClientProgramAction`). All routine/exercise/set editing moved to `useMyRoutinesStore`, keyed by `routine.id` alone, with its own independent debounced PATCH via `updateRoutineAction`. `RoutineExercise` only stores `exerciseId` (no embedded name/muscle) — display components resolve it via `useExerciseCatalogStore` (`src/lib/exercise-catalog-store.ts`), hydrated once app-wide from the `(main)` layout, rather than prop-drilling the catalog.
...
- **"Yaaro Coach Library" tab renamed to Explore (2026-08-13, corrected same day)** — Program Library keeps its original **3 tabs**: My Library (`?visibility=mine` → `createdById === me`), My Routines (same, routines), Explore (`?visibility=explore` → `visibility:"public" AND createdById !== me`). The 13 former templates are **not** a null-`createdById`/special-case row — they're ordinary public programs owned by one fixed real `user` account (`SEED_PROGRAM_OWNER_ID` in `yaaro_backend/.env`), so they land in Explore for every other coach via the exact same query as any coach-to-coach shared program — no separate template concept in the schema. All `yaaro-coach-*` template-browsing components/types/store were deleted and replaced by `explore-program-list.tsx`/`explore-empty-state.tsx`, which reuse the plain `Program` type. No Explore tab exists for routines — `routine.visibility` is a field but nothing browses public routines standalone yet.
- **Routine Editor reuses the app's real exercise catalog, not a coach-specific one.** The exercise picker calls `GET /mobile/v1/exercises` directly (same endpoint + data the mobile app's own "Choose exercise" screen uses) rather than a new `/coach/v1/exercises`. It takes no query params, so search/Equipment/Muscles filtering happens client-side over the ~430-exercise list in `lib/exercise-filters.ts`. Every routine exercise's `exerciseId` must now resolve to a real catalog entry — no more free-text/custom exercise names, as of the 2026-08-13 program/routine split.
...
- **2026-08-13**: Split `program` (previously embedding `routines[]`→`exercises[]` as Mongoose subdocuments) into two separate top-level collections, `program` and `routine`, linked by `program.routineIds: [ObjectId]`. Removed the read-only `programTemplate` model entirely — a "template" is now just a `program` doc with `visibility: "public"` (new field, also added to `routine`) owned by someone other than the viewing coach. Richer per-exercise shape: `exerciseId` (ref into the real exercise catalog, now **required** — no more free-text custom exercise names), `restSeconds` (Number, was a free-text `rest` string), `actions: [{key, data}]` (was `[String]`), `set: [{metrics: [{type, value}]}]` (was `sets: [{lbs, reps}]`, `metrics[].type` is a deliberately free-form string so new metric types never need a schema change). Three follow-on decisions made with the user before implementing: (1) `/coach/v1/programs*` now requires `authenticateCoachToken` — the `DEFAULT_COACH_ID` stub is gone, `createdById` is always `req.coachToken.id`; (2) routines are **deep-cloned, never shared by reference**, whenever they cross an ownership boundary (assigning a program to a client, duplicating a public program, adding someone else's public routine to your library) — but linking one of *your own* routines into a second of your own programs is a genuine shared reference by design; (3) `exerciseId` is unconditionally required, no custom-exercise fallback. Pre-launch schema, no data migration needed/done.
  - **yaaro_backend**: rewrote `program.js`; renamed `coach_routine.js` → `routine.js` and rewrote it; deleted `program_template.js` + its controller/route; rewrote `program_ctrl.js` (drops template-clone, adds `sourceProgramId` duplicate-with-clone, visibility-aware `$or` list/get filtering), `routine_ctrl.js` (added `getById`/`update`), `client_ctrl.js`'s `assignProgram`/`getProgram`/`updateProgram` (clones referenced routines, added orphaned-private-routine cleanup on program removal/reassignment); rewrote the hand-maintained Joi validators to match; added `VISIBILITY`/`PROGRAM_LEVEL`/`PROGRAM_GOAL`/`PROGRAM_EQUIPMENT` constants (mirroring the existing `CLUB_VISIBILITY` pattern); renamed+rewrote `scripts/seed-program-templates.js` → `scripts/seed-public-programs.js` (`npm run seed:publicPrograms`, now requires `SEED_PROGRAM_OWNER_ID`).
  - **yaaro_coach**: rewrote `program-editor` types (`Program`/`Routine`/`RoutineExercise` replace `Program`/`ProgramRoutine`/`ProgramExercise`/`CoachRoutine`); split `useMyProgramsStore` (program-level fields + `routineIds` only) from `useMyRoutinesStore` (now the single home for all routine/exercise/set editing, keyed by `routine.id`); new `useExerciseCatalogStore` + `(main)`-layout hydrator so any component can resolve an `exerciseId` without prop-drilling the catalog; kept the Program Library's 3 tabs but renamed the third from "Yaaro Coach Library" to "Explore" (`?visibility=explore` → public-and-not-mine; the 13 templates are just public programs owned by one fixed real account, no null-`createdById` special case); deleted every `yaaro-coach-*` template-browsing file (components, store, types, data) now that templates are just public programs; reworked the Routine Editor's rest-timer (seconds-based) and set/metric columns (driven by each exercise's `actions[]`, one metric type per action key, no hardcoded lbs/reps).
  - Verified: `tsc --noEmit` and `eslint` clean across the whole changed surface; backend models/routes load and the server boots cleanly against the real dev DB (`node -e "require(...)"` + a timed `node ./bin/www` run). **Not verified**: an actual click-through in a running browser against seeded data (no `SEED_PROGRAM_OWNER_ID` set in this environment, so the public-program seed wasn't run) — static verification only this round.
  - **Known pre-existing doc drift, not addressed this session**: §3's folder structure and parts of §8/§10 (e.g. "no client/coach relationship model") were already stale going into this session (the repo now has a `team` feature, a `user_coach`/client-roster model, a full Client Detail page with real backend wiring, etc., none reflected there) — a fuller doc sync is a separate task.
