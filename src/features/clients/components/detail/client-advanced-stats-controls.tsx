  ...
  return (
    <div className="flex items-center gap-2">
      <Select
        value={granularity}
        onValueChange={(value) =>
          onGranularityChange(value as AdvancedStatsGranularity)
        }
      >
        <SelectTrigger className="w-40">
          <Clock className="size-4 text-muted-foreground" />
          <SelectValue>
            {(value: AdvancedStatsGranularity) => GRANULARITY_LABELS[value]}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
      ...
      <Select
        value={range}
        onValueChange={(value) => onRangeChange(value as AdvancedStatsRange)}
      >
        <SelectTrigger className="w-44">
          <Calendar className="size-4 text-muted-foreground" />
          <SelectValue>
            {(value: AdvancedStatsRange) => RANGE_LABELS[value]}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
