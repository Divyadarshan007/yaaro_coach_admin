...
export async function createMembershipPlan(input: CreateMembershipPlanInput): Promise<MembershipPlan> {
  ...
  if (!res.ok) return parseApiError(res, "Failed to create membership plan");
...
export async function updateMembershipPlan(
  ...
  if (!res.ok) return parseApiError(res, "Failed to update membership plan");
  return normalizePlan(await res.json());
}
...
export async function deleteMembershipPlan(id: string): Promise<void> {
  ...
  if (!res.ok) await parseApiError(res, "Failed to delete membership plan");
}
